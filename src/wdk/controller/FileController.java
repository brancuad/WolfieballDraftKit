...
public class FileController {
    ...
    // FOR READING AND WRITING DRAFT DATA
    private DraftFileManager draftIO;
    
    // FOR EXPORTING THE DRAFT SUMMARY PAGE
    ...
    // WORK BY THE DRAFT HAS COMPLETED
    ...
    /**
     * This default constructor starts the program without a draft file being edited.
     * 
     ...
     * data.
     * @param initExporter The object that will be exporting drafts to Web sites.
    */
    ...
    public void handleNewDraftRequest(WDK_GUI gui){
        try {
            ...
            if (!saved) {
                // THE USER CAN OPT HERE WITH A CANCEL
    ...
    /**
     * This method lets the user open a Draft saved to a file.  
     * It will also make sure data for the current Draft is not lost.
     * 
     * @param gui The user interface editing the draft.
     */
     ...
     * know the name of the file, so we won't need to prompt the uesr.
     * 
     * @param gui The user interface editing the Draft.
     ...
     * the open process, nothing is done.  If an error occurs loading the file,
     * a message is displayed, but nothing changes.
     */
